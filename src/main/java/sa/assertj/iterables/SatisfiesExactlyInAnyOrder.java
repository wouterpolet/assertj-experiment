...
public class SatisfiesExactlyInAnyOrder extends Experiment {
    static Experiment.DataProvider provider = (size) -> {
        ...
        Collections.shuffle(iterable_list);
        Collections.shuffle(consumer_list);
        return new Object[]{iterable_list, consumer_list.toArray(Consumer[]::new)};
    };

    static Experiment.AssertionRunner runner = s -> assertThat((ArrayList<Integer>) s[0]).satisfiesExactlyInAnyOrder((Consumer<Integer>[]) s[1]);

    public SatisfiesExactlyInAnyOrder() {
